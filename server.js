...
const isAuthenticated = ({ email, passwords }) => {
  return (
    getUsersDb().users.findIndex(
      user => user.email === email && user.password === passwords
    ) !== -1
  );
};
const isExist = email => {
...
server.post('/auth/login', (req, res) => {
  ...
  if (isAuthenticated({ email, passwords })) {
    const user = getUsersDb().users.find(
      u => u.email === email && u.password === passwords
    );
